...
public class Perfilometro {
    ...
    public static ArrayList<AlunoContinuo> getPerfis(ArrayList<DKGObjeto> perfils, ArrayList<SemanaContinua> semanas) {
        ...
        for (DKGObjeto aluno_objeto : perfils) {
            ...
            perfil.setSID(aluno_objeto.identifique("ID").getValor());
            // perfil.setNome(aluno_objeto.identifique("Nome").getValor());
            //  perfil.setNota(aluno_objeto.identifique("NotaComRecuperacao").getValor());

            // System.out.println(aluno_objeto.listar());
            ...
            if (aluno_objeto.identifique("RecuperacaoRealizada").isValor("SIM")) {
                ...
                SemanaContinuaValores rec = new SemanaContinuaValores(rData, rData, rValor, Texto.doubleNumC2(String.valueOf(rGanhou)), "RECUPERAÇÃO", rValor);
